...
static int mx_check_data(int nbytes, char **data, const int fd) {
	if (nbytes == 0 && (!data[fd] || data[fd][0] == '\0'))
		return -1;
	return 0;
}
static int mx_read(char **data, const int fd, size_t buf_size, char delim) {
	char buf[buf_size];
	char *temp = NULL;
	int nbytes = 0;
...
int mx_read_line(char **lineptr, size_t buf_size, char delim, const int fd) {
	...
	nbytes = mx_read(data, fd, buf_size, delim); 
	if ((mx_check_data(nbytes, data, fd) == -1))
			return -1;
	return mx_count_bytes(data, lineptr, delim, fd);
}
